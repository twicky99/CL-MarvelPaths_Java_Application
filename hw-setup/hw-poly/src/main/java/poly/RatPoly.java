...
public final class RatPoly {
    ...
    public RatPoly(RatTerm rt) {
        terms = new ArrayList<RatTerm>();
        //If rt is not 0 and rt's is not negative, rt is included in RatPoly
        if(!(rt.getCoeff().doubleValue() == 0)&&(rt.getExpt() >= 0)){
        terms.add(rt);}
        checkRep();
    }
    ...
    public int degree() {
        ...
        // Inv: {{maxPower = max of terms from 0 to step,0<=step<terms.size()}}
        for(int step = 0; step<terms.size();step++){
            RatTerm item = terms.get(step);
            // If the RatTerm id not zero and it's exponent is larger than the
            // max then replace the max with this RatTerm's exponent
        if((item.getExpt()>maxPower)&&(item.getCoeff().doubleValue() != 0)){
            maxPower = item.getExpt();
        }
        }
        checkRep();
        return maxPower;
    ...
    public RatTerm getTerm(int deg) {
        ...
        RatNum zer0 = new RatNum(0);
        RatTerm zero = new RatTerm(zer0,1);
        return zero;
    }
    ...
    private static void scaleCoeff(List<RatTerm> lst, RatNum scalar) {
        ...
        for(int i = 0; i<lst.size();i++){
            RatNum p= scalar.mul(lst.get(i).getCoeff());
            RatTerm pro = new RatTerm(p,lst.get(i).getExpt());
            lst.remove(i);
            lst.add(i,pro);
        }
    }
    ...
    private static void sortedInsert(List<RatTerm> lst, RatTerm newTerm) {
        if (!newTerm.isZero()) {
            ...
            int place = 0;
            int term = newTerm.getExpt();
            boolean work = true;
            for (int i = 0; i < lst.size(); i++) {
                if (lst.get(i).getExpt() == newTerm.getExpt()) {
                    RatNum rep = lst.get(i).getCoeff().add(newTerm.getCoeff());
                    lst.remove(i);
                    RatTerm repl = new RatTerm(rep, newTerm.getExpt());
                    if(!repl.isZero()){
                    lst.add(i, repl);}
                    work = false;
    ...
    public RatPoly add(RatPoly p) {
        ...
        Collections.reverse(p.terms);
        List<RatTerm> goal = new ArrayList<>();
        ...
        for(int i = 0;i<max;i++){
            ...
            if(terms.size() > i){
                sortedInsert(goal,terms.get(i));
            }
            if(p.terms.size() > i){
                sortedInsert(goal,p.terms.get(i));
            }
        }
        Collections.reverse(p.terms);
        Collections.reverse(terms);
        return new RatPoly(goal);
    }
    ...
    public RatPoly mul(RatPoly p) {
        ...
        RatPoly goal = new RatPoly();
        //Inv {{goal = terms_0*p.terms+...+terms_step*p.terms}}
        for(int step = 0; step< terms.size();step++){
            //Inv {{goal = terms_0*p.terms_step2+...+terms_step*p.terms_2}}
        for(int step2 = 0; step2< p.terms.size();step2++){
            RatPoly pop = new RatPoly(terms.get(step).mul(p.terms.get(step2)));
            goal.add(pop);
        }
        }
        return goal;
    ...
    public RatPoly differentiate() {
        ...
        for(RatTerm term : terms) {
            RatTerm newTerm = term.differentiate();
            if(!newTerm.isZero()) {
                this.sortedInsert(newTerms, newTerm);
            }
        }
        return new RatPoly(newTerms);
    }
    ...
    public RatPoly antiDifferentiate(RatNum integrationConstant) {
        ...
        List<RatTerm> newTerms = new ArrayList<RatTerm>();
        for(RatTerm term : terms) {
            RatTerm newTerm = term.antiDifferentiate();
            if(!newTerm.isZero()) {
                this.sortedInsert(newTerms, newTerm);
            }
        }
        if(!integrationConstant.equals(RatNum.ZERO)) {
            this.sortedInsert(newTerms, new RatTerm(integrationConstant, 0));
        }
        return new RatPoly(newTerms);
    }
    ...
    public double integrate(double lowerBound, double upperBound) {
        RatPoly p = this.antiDifferentiate(RatNum.ZERO);
        return p.eval(upperBound) - p.eval(lowerBound);
