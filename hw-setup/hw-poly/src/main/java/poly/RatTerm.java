...
public final class RatTerm {
    ...
    public RatTerm add(RatTerm arg) {
        // If the exponents are not equal without either number being 0 or NaN then throw new IllegalArgumentException.
        if((expt != arg.expt) && this.isNaN() && arg.isNaN() && this.isZero() && arg.isZero()){
            throw new IllegalArgumentException("neither argument is zero or NaN!");
        }
        ...
        RatTerm goal = new RatTerm(coeff.add(arg.coeff),expt);
        return goal;
    }
    ...
    public RatTerm sub(RatTerm arg) {
        ...
        RatTerm goal = new RatTerm(coeff.sub(arg.coeff),expt);
        return goal;
    }
    ...
    public RatTerm mul(RatTerm arg) {
        if(arg == NaN || coeff.isNaN()){
            return NaN;
        }
        RatTerm goal = new RatTerm(coeff.mul(arg.getCoeff()),expt+arg.getExpt());
        return goal;
    }
    ...
    public RatTerm div(RatTerm arg) {
        ...
        RatTerm goal = new RatTerm(coeff.div(arg.getCoeff()),expt-arg.getExpt());
        return goal;
    }
    ...
    public RatTerm differentiate() {
        ...
        RatNum tr = new RatNum(expt,1);
        RatTerm goal = new RatTerm(coeff.mul(tr),expt-1);
        return goal;
    }
    ...
    public RatTerm antiDifferentiate() {
        ...
        RatNum tr = new RatNum(expt+1,1);
        RatTerm goal = new RatTerm(coeff.div(tr),expt+1);
        return goal;
    }
